...
function GuideJsonLd({
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Guides", item: "https://forge.brgt.site/guides" },
      { "@type": "ListItem", position: 3, name: guide.title, item: `https://forge.brgt.site/guides/${guide.slug}` },
    ],
  };
...
// ── MDX Components ────────────────────────────────────
// Custom renderers for MDX content — styled to match Forge design system
const mdxComponents = {
    ...
    <>
      ...
      <header className="border-b border-border bg-bg/80 backdrop-blur-md sticky top-0 z-50">
        ...
        >
          ...
          <Link href="/guides" className="text-text-muted text-sm hover:text-text-base transition-colors">
            guides
          </Link>
          <span className="text-border">/</span>
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        ...
        <header className="mb-10" aria-labelledby="guide-title">
          ...
          <div className="flex items-center gap-4 mt-6 pt-5 border-t border-border text-xs font-mono text-text-faint">
            <span>Published {new Date(guide.publishedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</span>
            {guide.updatedAt && (
              <span>Updated {new Date(guide.updatedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</span>
            )}
          </div>
        </header>
        ...
        {/* MDX Content */}
        <article
          className="prose-forge max-w-none"
